...
test('video deletion removes only the confirmed, unchanged file and persists the index', async () => {
  ...
  try {
    ...
    const remove = (id, confirm = 'delete-from-disk') => fetch(`${url}/api/movie`, { method: 'DELETE', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ id, confirm }) });
    assert.equal((await remove(retained.id, '')).status, 400);
    assert.equal(await readFile(retained.path, 'utf8'), 'fixture');
    assert.equal((await remove('unknown')).status, 404);
    await writeFile(other.path, 'replacement video');
    assert.equal((await remove(other.id)).status, 400);
    assert.equal(await readFile(other.path, 'utf8'), 'replacement video');
    assert.equal((await remove(retained.id)).status, 200);
    await assert.rejects(stat(retained.path), { code: 'ENOENT' });
    assert.equal(await readFile(sibling.path, 'utf8'), 'fixture');
    const index = await (await fetch(`${url}/api/library`)).json();
    assert.ok(!index.movies.some((movie) => movie.id === retained.id));
    const saved = JSON.parse(await readFile(join(app, '.cache/index.json'), 'utf8'));
    assert.ok(!saved.movies.some((movie) => movie.id === retained.id));
    assert.equal((await remove(retained.id)).status, 404);
  } finally {
