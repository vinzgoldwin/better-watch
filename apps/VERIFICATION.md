...
## Repository layout verification: 2026-09-22

Platform source now lives in `apps/mac/`, `apps/ipad/`, `apps/android/`, and `apps/tv/`. Phone and TV retain one Android APK. iPad still compiles the Mac model and synchronization source directly.

- Android: `testDebugUnitTest assembleDebug assembleDebugAndroidTest lintDebug` passed on Asahi with the isolated synthetic fixture. All 9 phone/TV local UI tests passed. Lint reported zero errors and 25 warnings; splitting TV resources exposes a separate density-folder warning for the existing xhdpi TV banner. No resource content or runtime behavior changed for the folder move.
- Mac: `apps/mac/build-app.sh` and `swift test --package-path apps/mac -c release` passed on the Apple host. The app bundle was built and its signature verified.
- iPad: XcodeGen resolved the relocated shared Swift files, and an unsigned iOS Simulator build succeeded.
- Server: all 34 Node/media tests passed using synthetic fixtures.

Apple validation used a temporary source-only directory because the normal Mac checkout has uncommitted work; it was not overwritten. Android source and tooling stayed on Asahi. No apps were installed or launched on personal devices, and no production server restart or personal-library mutation was performed. These build/local-test results do not replace the pending physical TV verification in [tv/VERIFICATION.md](tv/VERIFICATION.md).
