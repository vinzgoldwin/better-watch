...
The same APK also includes a dedicated **Google TV / Android TV** interface. See [TV installation, remote controls, and verification](../tv/README.md). Phone and TV share LibraryModel, filtering, and PlaybackSession; their Compose interfaces and launcher activities are separate.

This directory owns the Gradle build, shared manifest, phone UI, library model, and playback. TV's UI, banner, and tests live in `../tv/` and are included by the app's Gradle source sets.
