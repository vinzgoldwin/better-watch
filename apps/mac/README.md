...
The build script creates and ad-hoc signs `apps/mac/.build/Better Watch.app`. Quit the installed app before copying a new build to `~/Applications/Better Watch.app` with `ditto`. The original launcher was preserved at `~/Library/Application Support/Better Watch/Previous Launcher.app`.
